...
export const searchService = {
    searchByKeyword: async (keyword, type = 'all') => {
        const handlers = {
            artifact: async () => (await searchRepository.findArtifactsByKeyword(keyword)).map((a) => ({
                id: a.id,
                entityName: a.title,
            })),
            scrap: async () => (await searchRepository.findScrapsByKeyword(keyword)).map((s) => ({
                id: s.id,
                entityName: s.title,
            })),
            user: async () => (await searchRepository.findUsersByKeyword(keyword)).map((u) => ({
                id: u.id,
                entityName: u.userName,
            })),
            tag: async () => (await searchRepository.findTagsByKeyword(keyword)).map((t) => ({
                id: t.id,
                entityName: t.name,
            })),
        };
    ...
    searchArtifacts: async (keyword) => {
        return Result.succeed((await searchRepository.findArtifactsByKeyword(keyword)).filter(isPublished));
    },
    searchScraps: async (keyword) => {
