...
export const authService = {
    ...
    loginWithGoogle: async (input) => {
        ...
        if (!input.email) {
            return Result.fail(new Error('Google account does not have an email address.'));
        }
        // --------------------------------------------------
        ...
        if (existingEmailUser) {
            ...
            const updatedUser = await authRepository.updateGoogleId(existingEmailUser.id, input.googleId);
            return Result.succeed({
                user: {
                    id: updatedUser.id,
