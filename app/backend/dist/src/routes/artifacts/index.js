...
import { artifactSchema, getArtifactsQuerySchema, registerArtifactSchema, updateArtifactSchema, } from './schema.js';
export const artifacts = new Hono()
...
}), authCheck, validator('json', registerArtifactSchema), async (c) => {
...
}), authCheck, validator('query', getArtifactsQuerySchema), async (c) => {
    const userId = c.var.user.userId;
    const query = c.req.valid('query');
    const isFollowing = query?.isFollowing;
    const result = await artifactsService.getArtifacts(query, isFollowing !== undefined
        ? isFollowing
            ? { userId }
            : undefined
        : undefined);
    ...
    .patch('/:artifactId', describeRoute({
    tags: ['artifacts'],
    description: 'Get artifact detail',
    responses: {
        200: {
...
}), authCheck, validator('json', updateArtifactSchema), async (c) => {
    const { artifactId } = c.req.param();
    const userId = c.var.user.userId;
    const { tagIds, ...data } = c.req.valid('json');
    ...
    .delete('/:artifactId', describeRoute({
    tags: ['artifacts'],
    description: 'Get artifact detail',
}), authCheck, async (c) => {
    const { artifactId } = c.req.param();
    ...
    .post('/:artifactId/summary', describeRoute({
    tags: ['artifacts'],
