...
import { artifactSearchResultSchema, scrapSearchResultSchema, searchQuerySchema, tagSearchResultSchema, userSearchResultSchema, } from './schema.js';
export const search = new Hono()
    .get('/artifact', describeRoute({
    tags: ['Search'],
...
}), validator('query', searchQuerySchema.pick({ keyword: true })), async (c) => {
    const { keyword } = c.req.valid('query');
    const result = await searchService.searchArtifacts(keyword);
    if (result.type === 'Failure') {
        return c.json({ message: 'Unexpected error occurred during search.' }, 500);
    }
    return c.json(result.value);
})
    .get('/scrap', describeRoute({
    tags: ['Search'],
