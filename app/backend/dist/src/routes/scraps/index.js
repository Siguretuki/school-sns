...
import { getScrapsQuerySchema, registerScrapSchema, scrapSchema, updateScrapSchema, } from './schema.js';
...
}), authCheck, validator('json', registerScrapSchema), async (c) => {
...
}), authCheck, validator('query', getScrapsQuerySchema), async (c) => {
    const userId = c.var.user.userId;
    const query = c.req.valid('query');
    const isFollowing = query?.isFollowing;
    const result = await scrapsService.getScraps(query, isFollowing !== undefined && isFollowing ? { userId } : undefined);
    if (result.type === 'Failure') {
        return c.json({
...
}), authCheck, validator('json', updateScrapSchema), async (c) => {
    const { scrapId } = c.req.param();
    const date = c.req.valid('json');
    const userId = c.var.user.userId;
    const result = await scrapsService.updateScrap(scrapId, userId, {
        title: date.title,
        body: date.body,
    }, date.tagIds);
    if (result.type == 'Failure') {
